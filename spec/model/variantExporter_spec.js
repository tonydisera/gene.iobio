...
var theGene = {gene_name: 'RAI1', chr: '17', start: 17584787, end: 17714767, strand: '+', transcripts: [theTranscript] };
...
describe('variantExporter', function() {
	...
	var bookmarkEntries = [
 	 {chrom: 'chr17',start: 17698535	,end: 17698536	,ref: 'G'	,alt: 'A'	,gene: 'RAI1'	,transcript: 'ENST00000353383.1' ,freebayesCalled: '' },
	 {chrom: 'chr20',start: 30409363	,end: 30409364	,ref: 'A'	,alt: 'G'	,gene: 'MYLK2'	,transcript: 'ENST00000375994.2' ,freebayesCalled: 'Y'},
	 {chrom: 'chr22',start: 39636863	,end: 39636865	,ref: 'GCC'	,alt: 'G'	,gene: 'PDGFB'	,transcript: 'ENST00000331163.6' ,freebayesCalled: ''},	
	 {chrom: 'chrX'	,start: 19369471	,end: 19369472	,ref: 'G'	,alt: 'T'	,gene: 'PDHA1'	,transcript: 'ENST00000379806.5' ,freebayesCalled: ''}		
	];
	...
	beforeEach(function(done) {
		...
		model.onVcfUrlEntered('https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome.vcf.gz', function(success, samples) {
			model.setSampleName('NA12878');
			done();
		});
	});

	describe('#exportBookmarkedVariants', function() {
		beforeEach(function(done) {
			model.promiseGetVariants(theGene, theTranscript, theGene.start, theGene.end).then(function(theVcfData) {
				done();
			})
		});

		it('exports bookmarked variants', function(done) {

			expect(variantExporter).not.toBeNull();
			expect(model).not.toBeNull();

			expect(model.sampleName).toEqual('NA12878');
			expect(model.vcfData).not.toBeNull();
			done();
		});
	});
});